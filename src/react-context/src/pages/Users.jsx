...
export default function Users() {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - state.userList.length) : 0;

  const filteredUsers = applySortFilter(state.userList, getComparator(order, orderBy), filterName);
    ...
    <Page title="Users | Definity First University">
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" component={RouterLink} to="/dashboard/users/00000000-0000-0000-0000-000000000000" startIcon={<Icon icon={plusFill} />}>
            New User
          </Button>
        </Stack>
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={isItemSelected} aria-checked={isItemSelected}>
                        ...
                        <TableCell align="left">
                          <Label variant="ghost" color={(status === "lazy" && "error") || "success"}>
                            {sentenceCase(status)}
                          </Label>
                        </TableCell>
