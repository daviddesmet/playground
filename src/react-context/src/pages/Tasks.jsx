...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_task) => _task.name.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function Users() {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - state.taskList.length) : 0;

  const filteredTasks = applySortFilter(state.taskList, getComparator(order, orderBy), filterName);
    ...
    <Page title="Tasks | Definity First University">
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" component={RouterLink} to="/dashboard/tasks/00000000-0000-0000-0000-000000000000" startIcon={<Icon icon={plusFill} />}>
            New Task
          </Button>
        </Stack>
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={isItemSelected} aria-checked={isItemSelected}>
                        ...
                        <TableCell align="right">
                          <TableMoreMenu route={id} onDeleteClick={() => handleDeleteOne(id)} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
