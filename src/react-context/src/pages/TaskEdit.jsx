...
import { Autocomplete, Breadcrumbs, Card, CardContent, Container, Divider, Link, TextField, Typography, Stack, Switch } from "@material-ui/core";
...
export default function TaskEdit() {
  ...
  const TaskSchema = Yup.object().shape({
    name: Yup.string().min(2, "Too Short!").max(100, "Too Long!").required("Task required"),
    user: Yup.object()
      .shape({
      ...
      <Container>
        <Stack direction="column" alignItems="flex-start" justifyContent="space-between" mb={5}>
          ...
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">{isNewTask(id) ? "New" : "Edit"} Task</Typography>
          </Breadcrumbs>
        </Stack>
                ...
                <Stack spacing={3}>
                  ...
                  <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                    <Autocomplete
                      ...
                      isOptionEqualToValue={(option, value) => (value === "" ? true : option.id === value?.id)}
                      options={users}
                      onChange={handleUserOptionChange}
                      value={values.user || null}
                      renderInput={(params) => (
                        <TextField {...params} label="User" error={Boolean(touched.user && errors.user)} helperText={touched.user && errors.user} />
                      )}
                    />
                    <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2} margin={3} pl={6}>
                      <Typography variant="body2">Completed</Typography>
                      <Switch color="primary" {...getFieldProps("completed")} checked={values.completed} inputProps={{ "aria-label": "controlled" }} />
                    </Stack>
                  </Stack>
