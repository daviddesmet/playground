        ...
        <v-card class="elevation-12">
          ...
          <v-form @submit.prevent>
            <v-card-text>
              ...
              <v-spacer></v-spacer>
              <v-text-field prepend-icon="verified_user" name="code" label="Verification Code" type="text" v-model="authenticator.code" @keyup.enter="submit" autofocus></v-text-field>
            </v-card-text>
  ...
  private created() {
    ...
    this.$store
      ...
      .then((result: any) => {
        ...
        (this.$refs.qr as Vue & { update: (opts: any) => void }).update(
          this.options
        );
      })
      .catch(err => {
